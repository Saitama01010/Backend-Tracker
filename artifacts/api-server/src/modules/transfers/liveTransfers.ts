...
export type LiveTransferRefreshRequestResult =
  | { status: "started" }
...
export async function requestLiveTransferRefresh(userId: number): Promise<LiveTransferRefreshRequestResult> {
  ...
  try {
    await postgresBackgroundJobStore.enqueue({
      ...
      idempotencyKey: manualJobKey("live_transfer_refresh", userId),
