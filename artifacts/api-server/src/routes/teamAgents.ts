...
let notesColumnReady: Promise<unknown> | null = null;

function ensureNotesColumn() {
  notesColumnReady ??= db.execute(sql`ALTER TABLE "team_agents" ADD COLUMN IF NOT EXISTS "notes" text`);
  return notesColumnReady;
}
...
router.post("/team-agents", requireAuth, requireRole("admin"), async (req, res) => {
  await ensureNotesColumn();
  const { name, team, arabicName, shift, notes } = req.body ?? {};
...
router.patch("/team-agents/:id", requireAuth, requireRole("admin"), async (req, res) => {
  ...
  const { name, team, active, arabicName, shift, notes } = req.body ?? {};
